...
=====================================
Drives a generic 5x5 WS2812b (NeoPixel) matrix via GP6 on the
RP2350 bit board, through the TTL signal level-shifter module.

PIN CHOICE: GP6 = micro:bit small edge pad 3 (verified in milestone 1).
  GP0 and GP1 are NOT used -- they are tied to Button A / Button B
  on the RP2350 bit board.

WIRING:
  RP2350 GP6 ──────► Level-Shifter DIN  (signal in, 3.3V)
  Level-Shifter DOUT ──► WS2812b DIN    (signal out, 5V)
  5V PSU (+)  ──────► WS2812b VCC + Level-Shifter high-voltage VCC (output side)
  GND (common) ─────► RP2350 GND + Level-Shifter GND + WS2812b GND + PSU GND

  ** Do NOT connect Level-Shifter low-voltage VCC (input side) to 3.3V --
     it self-biases to 5V from the output side. Connecting might overload the MCU's power rail! **
...
PIXEL LAYOUT:
  ...
  This script auto-detects by lighting pixels 0-4 (first five in the chain)
  so you can visually confirm which row/direction they follow.
...
# ---- Configuration ----
PIXEL_PIN = board.GP6  # micro:bit edge pad 3
NUM_PIXELS = 25  # 5 x 5
...
def xy_to_index(x, y):
    ...
    if SERPENTINE:
        if y & 1 == 0:  # mathematically the same as modulo 2, but faster, since it is bitwise and operation
...
def show_pattern(pattern_str, color=WHITE):
    """Display a 5x5 '#'/'. ' pattern string (same syntax as micro:bit MakeCode)."""
...
print("=" * 56)
print(f"  Pin : GP0   Pixels: {NUM_PIXELS}   Brightness: {BRIGHTNESS}")
