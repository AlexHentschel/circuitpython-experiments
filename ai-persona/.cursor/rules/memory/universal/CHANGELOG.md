...
**Change** (Content-Hierarchy **Level 3** foundational-rule refinement — made deliberately, at explicit human direction, with a stated rationale; reversible):

- **Invariant refined** (`00-memory-system.mdc § File Architecture` + the Layout blurb): the "project repos hold heavy artifacts, linked never copied (C8)" rule now carries **one documented exception** — a *meta project whose "repo" is the persona itself* (e.g. `ai-tooling`) has no external original, so its durable, re-runnable artifacts live **in-tree** under `projects/<slug>/lifecycle-kit/` and are pointed-to from `CONTEXT.md § Entry points` (never read at session start). Rationale: C8 guards against a memory-copy diverging from an external original; with no external original there is nothing to diverge from — the in-tree copy *is* the original. Scope-limited to durable re-runnable assets; ephemeral scratch still belongs in gitignored `ai-notes/`.
- **New `memory/projects/ai-tooling/lifecycle-kit/`** (`extend`): `README.md` (how-to-repeat + method pointers + evolved-persona adjustments), `launcher-prompt.md`, `harness/{run_probe.py,probe-pool-v1.md}`, `archive/{2026-09_EXPERIMENT-LOG.md,2026-09_final-probe-VERDICT.md}` — byte-identical copies lifted out of the gitignored `ai-notes/2026-09_CPy-full_memory_lifecycle/` before its cleanup. G10-sanitized (no stored key; `run_probe.py` reads `CURSOR_API_KEY` from env).
- **Repointed** ~10 durable pointers (this file, `projects/ai-tooling/{CONTEXT,SESSION_LOG}.md`, `concepts/ai-tooling.md`, `MONITORING.md`) from the `ai-notes/` path to the in-tree kit; purely historical mentions annotated, not chased.
- **Follows:** the gitignored `ai-notes/2026-09_CPy-full_memory_lifecycle/` scratch (`plan/`, `baseline/`, `eval/` traces, `ledger/`, HUMAN-REVIEW-DIGEST, WRAP-STATE) is now redundant with durable memory → human-run cleanup (`06`).

## 2026-09-15 — full memory-maintenance lifecycle (umbrella; branch `alex/ai-persona-maintanance`)

**Trigger:** Alex — "perform a full AI persona memory lifecycle" (task spec, preserved in-tree: `../projects/ai-tooling/lifecycle-kit/launcher-prompt.md`). This is the **umbrella entry** framing the whole 2026-09 cycle; the four sibling `2026-09-15` entries below (SESSION_LOG thinning · MONITORING P8 · P-prov header relocation · `ai-tooling` seed) are its children — not restated here. Session narrative: `projects/ai-tooling/SESSION_LOG.md § 2026-09-15 (lifecycle EXECUTED)`.
...
- **Verification:** final paired cold-AI probe (prior `8c4151b` vs candidate final HEAD) confirmed the central-log thinning left the warm-reset record reachable (candidate opened `SESSION_LOG.md` + `CHANGELOG.md` directly). See `../projects/ai-tooling/lifecycle-kit/archive/2026-09_final-probe-VERDICT.md`.
...
- New `projects/ai-tooling/{CONTEXT,SESSION_LOG}.md` (family `meta`; provisional) — workstream home for persona self-improvement + AI tooling. `CONTEXT.md § Entry points` references the launcher prompt (later preserved in-tree at `../projects/ai-tooling/lifecycle-kit/launcher-prompt.md` — see the 2026-09-15 umbrella entry) + the read-only HA method/harness source. Roster row + path-globs (`ai-persona/**`) added to `projects/_INDEX.md`. (commit `f4608f8`)
