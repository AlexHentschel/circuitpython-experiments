...
- **`asyncio.sleep(0)` still yields once** — not a true no-op; any `await` on it hands control back to the event loop once. Skipping the call entirely (vs. awaiting `sleep(0)`) is required for a genuinely synchronous/immediate return. Applied: exp16 `show_string()` fit-on-screen `interval_ms==0` branch.
...
- **CIRCUITPY deploy hygiene & the 2026-09-13 lib-wipe edge case** — never cancel a board copy mid-flight; clean-slate = REPL `storage.erase_filesystem()` (never host mass-rm the live volume); routine deploys via filtered scripts; single-master board access during copies. exp16 incident: whole `lib/` vanished (epoch-dated empty dirs) after an accidental mid-flight cancel + host-rm recovery round; S0-S3 bracketed repro **exonerated the toolchain** (don't re-run it); cause localized unproven (cancelled copy / host mass-rm / serial-attached-during-copy). **Recurrence resumption path inside the concept.**

## Candidate domains (NOT yet seeded — no concrete concept in evidence)

Per seed-on-evidence (C7): create when the first concrete concept arrives. From the rubric (`microcontroller-multi-project-memory-guidelines.md § 3.1`): `deep-sleep` (MCU sleep modes, wake sources, `alarm`), `display` (matrix render, column-major — the glyph-coordinate concept currently folded into `fonts`). `deep-sleep` is inherently cross-project — pollination pays once a 2nd project accrues content. **`tooling` seeded 2026-09-11** (first concept: Espressif 4MB TinyUF2 + esptool). **`led-driving` seeded 2026-09-12** (first concept: NeoPixel output peripheral per MCU family — PIO vs RMT); further WS2812 timing / level-shifting / current concepts (exp11/13/14 external strips via level shifter) accrue here. Remaining unseeded tooling-ish items (schemdraw, circup, VS Code) stay in `tooling.md`/this file until they accrue their own concepts.
