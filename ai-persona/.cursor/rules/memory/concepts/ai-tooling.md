...
These are expected to graduate into real `### concept` entries above once observed/verified *on this persona's own tooling*. They are recorded here only for discoverability; do not cite them as findings. Primary source for all: the lifecycle launcher prompt `ai-persona/ai-notes/2026-09_full_memory_lifecycle/2026-09_full_memory_lifecycle_prompt.md` (gitignored working store) + the HA methodology bundle above.
