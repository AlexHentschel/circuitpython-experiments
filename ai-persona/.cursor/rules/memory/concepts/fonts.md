...
**Rendering-code implication (exp14)**: `lib/display/core.py::_glyph_columns` and `_render_colmajor` are metric-agnostic — they read `font.ascent`, `font.descent`, per-glyph `width`, `height`, `dx`, `dy`, `shift_x` — so a font swap is a path-and-metadata change (update `_FONT_PATH`, re-verify metrics) with no rendering-code change required. The file `working-docs/font-distortion-probe.py` holds a reproducer of the current failure mode, useful as an A/B reference after the swap. Exp16 5×5 does **not** use this PCF path — see *DAL pendolino3 row-bytes vs column-major* below.

### DAL pendolino3 row-bytes vs Exp14 column-major

**Claim:** Lancaster DAL `MicroBitFont` / `pendolino3` stores each 5×5 glyph as **five row bytes**, with bit 4 = leftmost column and bit 0 = rightmost (`MicroBitFont.cpp` header). Exp14/Exp16 bitmap storage is **one byte per column**, bit N = row N (bit 0 = top). Conversion at vendor time: for row `r` and column `c`, if `(row_byte >> (4 - c)) & 1` then `col_bytes[c] |= 1 << r`. Status: `evidence-supported` (DAL header `!` example `{0x08,0x08,0x08,0,0x08}` = second column from left; host table in Exp16 `lib/display/font_makecode_5/`). Scope: `[domain:fonts]` `[cross-experiment]` (encoding); Exp16 tree is the first in-repo vendor (`[project:circuitpython-exp16-planetx]`).
