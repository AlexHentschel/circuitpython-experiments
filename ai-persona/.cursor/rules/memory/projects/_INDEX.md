# Projects — Roster (always read at session start)

Roster of all projects under this unified persona memory, with **path-globs for active-project detection (M1)**. At session start, match the most-recently-edited / open file path against the globs below to pick the active project; read only that project's folder (+ `universal/*`, `concepts/_INDEX.md`, central `SESSION_LOG.md`). If a path matches none or is ambiguous, **ask before writing per-project memory** (cross-contamination is the top failure mode). Workspace root for the CircuitPython family: `/Users/alex/Development/VsCode/CircuitPython/`.

> **Confirmed 2026-09-08.** Same retrieval/placement confirm as `concepts/_INDEX.md`. No recorded glob-ambiguity mis-route. Ask-if-ambiguous remains operational hygiene (not a remaining experiment). See `universal/CHANGELOG.md § 2026-09-08`. Dedicated-root (2026-07-15) is settled separately.

## Roster

| slug | title | family | path globs | memory home | status | last-touched | notes |
|------|-------|--------|-----------|-------------|--------|--------------|-------|
| `circuitpython-exp14-display` | Exp14 Display Library | `circuitpython` | `2026-04_Exp14_*/**` | `projects/circuitpython-exp14-display/` | active | 2026-06-12 | initial project at warm reset; Phase 3 in progress (branch `alex/display-mvp`) |
| `circuitpython-exp15-microbit` | Exp15 micro:bit alternative (RP2350) | `circuitpython` | `2026-06_Exp15_*/**` | `projects/circuitpython-exp15-microbit/` | active (early) | 2026-06-07 | Milestone-1 blink set up; on-device run pending board |
| `bamboo-lamp` | Bamboo-Lamp ambient lamp electronics | `bamboo-lamp` | `Bamboo-Lamp/**`, `/Users/alex/Projects/Family/Bamboo-Lamp/**` | `projects/bamboo-lamp/` | active | 2026-06-14 | unified into central at warm reset; technical artifacts stay in the Bamboo-Lamp repo, linked |
| `coding-tutor` | AI programming tutor for student "Alice" | `education` | `CodingTutor/**`, `/Users/alex/Development/VsCode/CircuitPython/CodingTutor/**` | `projects/coding-tutor/` | active (setup) | 2026-07-15 | "tutor development": build an AI tutor persona (distinct from this assisting persona) teaching CircuitPython on micro:bit+Nezha2+PlanetX. Heavy KB in the `CodingTutor/` repo. **Isana exemplar folders (`/Users/alex/Development/Isana/**`) are read-only reference, NOT a write-detection glob.** |
| `circuitpython-exp16-planetx` | Exp16 BPI-Bit-S2 CircuitPy PlanetX + LED | `circuitpython` | `2026-09_Exp16_*/**` | `projects/circuitpython-exp16-planetx/` | first milestone on-device (K1/K2/K3, 2026-09-13 code); font Phase 4 + `button_*` API host-green, Phase 5 and Stage-3 re-run pending | 2026-09-21 | LightTower PoC. UID `0740D10F1BE9` CP **10.3.0** (Path A; TinyUF2 0.35.0). Code `4a337f4`. Other boards not assumed. **Isana LightTower folder is read-only reference.** |
| `ai-tooling` | AI tooling & persona self-improvement | `meta` | `ai-persona/**`, `/Users/alex/Development/VsCode/CircuitPython/ai-persona/**` | `projects/ai-tooling/` | active (provisional) | 2026-09-15 | **First `meta`-family project.** Home for work *on this persona itself*: memory-maintenance lifecycles, harness/measurement tooling, rule/structure evolution. External method/harness source = `high-assurance-engineering` persona (read-only; not this persona's identity). Provisional structure — see `ai-tooling/CONTEXT.md`. |

**Residue-only experiments (no folder until content surfaces, C7)**: exp09 (`2026-02_Exp09_*`), exp11 (`2026-03_Exp11_*`), exp13 (`2026-03_Exp13_*`) — all `[family:circuitpython]`. Exp09 5×5 LUT/icons are prior art for exp16; still residue-only until they accrue their own memory content. Add a roster row + `projects/<slug>/` folder when they first accrue memory content.

## New-project-vs-extend heuristic (DN-MP-1 — refine after 3–5 projects of evidence)

Spin up a **new project** (new roster row + `projects/<slug>/CONTEXT.md`) when there is a new board **and** a new primary focus, or a new problem domain. Otherwise extend an existing project. Family is a **tag/column, not a directory** — group by the `family` column; do not create `families/` folders. Coarsening later (merging per-experiment projects into a themed one) is cheaper than splitting, so default to finest grain now (D1 = per-experiment + family).
