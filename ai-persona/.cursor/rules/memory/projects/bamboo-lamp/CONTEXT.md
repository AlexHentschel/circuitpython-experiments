...
> **SUPERSEDED 2026-07-15 by a design decision — NOT a test failure.** The symlink this section relied on was removed when the persona moved to a dedicated `ai-persona/` workspace root (no symlink-fanout — see `universal/CHANGELOG.md § 2026-07-15` and central `SESSION_LOG.md`). **Current model:** opening `Bamboo-Lamp/` standalone yields **zero** persona coverage; the persona loads only when `ai-persona` is in the open workspace. The empirical R-6 observation below remains true *as observed on 2026-06-14* and is retained verbatim as provenance.

The symlink `Bamboo-Lamp/.cursor/rules → <CircuitPython>/.cursor/rules` was **created + verified resolving 2026-06-14** (reaches all rule files + this digest). **Standalone-open test (R-6) PASSED 2026-06-14**: Bamboo-Lamp opened alone (CircuitPython workspace NOT attached) loaded `00`–`04` in the Rules panel and the unified `.cursor/rules/memory/` was fully reachable — Cursor follows the cross-tree symlink standalone. ~~R-6 fully closed; the user-level-root fallback is not needed. Standalone open is a supported entry point.~~ *(These forward-looking conclusions held only while the symlink existed; the symlink was removed by design 2026-07-15, so standalone open is no longer a supported entry point.)*
