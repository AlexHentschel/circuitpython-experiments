...
- **`lib/buttons.py`** — async onboard button-event API (`Button`, `OnboardButtons`). Not a fork of anything in Exp14.
...
- **Font-spacing build pipeline (2026-09-13 Phases 1–3, 2026-09-20 Phase 4 cutover)** — `scripts/dal_pendolino3.py` (shared DAL-fetch/convert helper, pinned commit SHA), `scripts/verify_dal_font_conversion.py` + `scripts/generate_spaced_font_table.py` (dev-machine-only, never deployed; fills `scripts/templates/spaced_glyphs.py.in` via stdlib `string.Template`), `lib/display/font_makecode_5/spaced_glyphs.py` (generated `[length, ink...]` table) + `ink.py` (`glyph_ink` accessor), `lib/display/text_layout.py` (`SpacedGlyphColumnFeeder`). Live `show_string` uses that feeder (always-spacer, space=3, tofu). `_COLUMN_MAJOR` / `glyph_columns` remain as the untrimmed table + host tests. Portable 8×8 rollout: re-run the same two-script pipeline against a new glyph source.
