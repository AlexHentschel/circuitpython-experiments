...
| Buttons A/B/C/D as press events | yes | yes | **yes** — lettered handlers stay; constructor seam is *which object* (`OnboardButtons` vs `PlanetXButtonSensor`) plus pin args |
...
| G2 | Constructors and config are the seam. Pins, LUT, font directory live in swap files and/or constructor args. Button wiring is two (or more) objects, not four pins on one dispatcher. |
