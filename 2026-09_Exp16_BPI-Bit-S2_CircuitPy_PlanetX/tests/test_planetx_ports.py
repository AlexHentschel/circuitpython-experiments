...
def test_j_port_silk_matches_nezha_then_goldfinger():
    """Jack silk P-numbers are ElecFreaks order, analog on J1/J2 only.

    - Covers: swapped P13/P14, analog assigned to J3, or GPIO numbers stored as silk.
    - How: ``J1.silk == (1, 8)`` … ``J3.silk == (13, 14)``; analog_silk only on J1/J2.
    """
    assert J1.name == "J1" and J1.silk == (1, 8) and J1.analog_silk == 1
    assert J2.name == "J2" and J2.silk == (2, 12) and J2.analog_silk == 2
    assert J3.name == "J3" and J3.silk == (13, 14) and J3.analog_silk is None
    assert J4.name == "J4" and J4.silk == (15, 16) and J4.analog_silk is None


def test_i2c_bus_is_shared_p19_scl_p20_sda():
    """All Nezha2 IIC jacks are one bus: P19 = SCL, P20 = SDA.

    - Covers: treating IIC as four independent buses, or swapping SCL/SDA silk.
    - How: ``I2C.silk_scl == 19`` and ``I2C.silk_sda == 20``; instance of ``I2CBus``.
    """
    assert isinstance(I2C, I2CBus)
    assert I2C.name == "I2C"
    assert I2C.silk_scl == 19
    assert I2C.silk_sda == 20
