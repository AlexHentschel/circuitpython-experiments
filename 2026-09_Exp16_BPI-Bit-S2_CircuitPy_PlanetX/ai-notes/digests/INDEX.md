...
**How to check:** every P0 row below (the four sources for the plan-refinement loop) has a file; if one is missing, collection is incomplete. Post-loop research digests (below the table) are not part of that gate.
...
Post-loop research digest (added 2026-09-07, after loop close — not a P0 gate row):

| File | Source (read, do not copy wholesale) | What a cold AI needs from it |
|------|--------------------------------------|------------------------------|
| `local-mcu-emulation.md` | Web docs scan: Wokwi, espressif QEMU, Renode, CP `unix` port | **Load only if** the pre-P8 emulation question is live. Verdict: no tool discharges K1/K2; Wokwi = only plausible smoke, CP-on-S2 **unverified**; P8 unchanged |
