...
for i, line in enumerate(lines):
    # Extract hex values
    parts = line.split('//')[0] # remove comment
    parts = parts.replace(',', '').split()
    hex_values = [int(x, 16) for x in parts]
    
    char_code = 32 + i
    char = chr(char_code)
    
    # Process into 5x5 grid (row-major)
    # 5 integers (cols), each integer 5 bits (rows, LSB=top)
    ...
    for r in range(5):
        ...
        for c in range(5):
            ...
            is_set = (val >> r) & 1
            if is_set:
                image_bytes.append(9)
            else:
                image_bytes.append(0)
                
    # Convert to bytes literal string
    bytes_lit = "b'"
    ...
    print(f"    {key_repr}: {bytes_lit}, # {char}")
